...
(function (root, factory) {
    ...
    var stub = function render(name, obj) {
        ...
        var tmp   = stub.templates[name].cloneNode(true);
        var items = tmp.getElementsByTagName("*");
        for (var i = items.length; i--;) {
            stub.handle_attributes(items[i],obj);
            stub.handle_textnodes(items[i],obj);
        }
        stub.handle_attributes(tmp,obj);
        stub.handle_textnodes(tmp,obj);
        return tmp;
    };
    ...
    stub.load = function() {
        ...
        /** Remove from DOM **/
        for (var key in stub.templates) {
            var e = stub.templates[key];
